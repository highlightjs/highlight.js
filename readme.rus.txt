...
При вызове таким образом highlight.js динамически подгружает соответствующие
языковые файлы и весит меньше, чем с набором по умолчанию, который включает 
многие распространенные языки. Поэтому же явное указание языков необходимо,
если нужный язык не входит в распространенные (см. DEFAULT_LANGUAGES в файле
highlight.js).
