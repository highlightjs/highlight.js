...
подсвечивать блоки кода динамически, читайте раздел «Инициализация вручную» ниже.
...
- Стилевые темы можно найти в загруженном архиве или так же использовать
...
фиксированное количество пробелов или отдельный `<span>`, чтобы задать ему
какой-нибудь специальный стиль:

```html
<script type="text/javascript">
  hljs.configure({tabReplace: '    '}); // 4 пробела
  // ... или
...
довольно простой эвристики: программа пытается подсветить фрагмент всеми
...
Это может понадобиться, например, на сайте, в котором нельзя подключить сам скрипт
highlight.js.
