...
function(hijs) {
    ...
    // Data step and PROC SQL statements; built-in functions
    var SAS_KEYWORDS = ' '+
        'do if then else end until while '+
        ' '+
        'abort array attrib by call cards cards4 catname '+
        'continue datalines datalines4 delete delim delimiter '+
        'display dm drop endsas error file filename footnote '+
        'format goto in infile informat input keep '+
        'label leave length libname link list lostcard '+
        'merge missing modify options output out page '+
    ...
    // User-defined macro functions (taken from tex.js)
    var SAS_MACRO_USR = {
        className: 'tag',
