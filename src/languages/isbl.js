...
function(hljs) {
  ...
  // Общий паттерн для определения имен функций
  var FUNCTION_NAME_IDENT_RE = '[A-Za-zА-Яа-яёЁ_][A-Za-zА-Яа-яёЁ_0-9]*';
  
  // keyword : ключевые слова
  var KEYWORD = 'and и else иначе endexcept endfinally endforeach конецвсе endif конецесли endwhile конецпока ' + 
  ...
  // Base constant
  var base_constants = 
  'CR FALSE nil NO_VALUE NULL TAB TRUE YES_VALUE ';
  ...
  'RESULT_VAR_NAME RESULT_VAR_NAME_ENG ';
  ...
  // Константы ==> built_in
  var CONSTANTS = sysres_constants + base_constants + 
  base_group_name_constants + decision_block_properties_constants +
  file_extension_constants + job_block_properties_constants +
  language_code_constants + launching_external_applications_constants +
  link_kind_constants + lock_type_constants + 
  monitor_block_properties_constants + notice_block_properties_constants +
  object_events_constants + object_params_constants + other_constants +
  privileges_constants + pseudoreference_code_constants + 
  requisite_ISBCertificateType_values_constants + requisite_ISBEDocStorageType_values_constants + 
  requisite_compType2_values_constants + requisite_name_constants +
  result_constants + rule_identification_constants + 
  script_block_properties_constants + subtask_block_properties_constants +
  system_component_constants + system_dialogs_constants +
  system_reference_names_constants + table_name_constants + 
  test_constants + using_the_dialog_windows_constants + 
  using_the_document_constants + using_the_EA_and_encryption_constants + 
  using_the_ISBL_editor_constants + wait_block_properties_constants + 
  sysres_common_constants;
  
  // enum TAccountType
  var TAccountType = 
  'atUser atGroup atRole ';
  ...
  var TCertificateInvalidationReason =
  'cirCommon cirRevoked ';

  // enum TCertificateType
  var TCertificateType = 
  'ctSignature ctEncode ctSignatureEncode ';

  // enum TCheckListBoxItemState
  var TCheckListBoxItemState = 
  'clbUnchecked clbChecked clbGrayed ';
  ...
  var TConditionFormat = 
  'cfInternal cfDisplay ';

  // enum TConnectionIntent
  var TConnectionIntent = 
  'ciUnspecified ciWrite ciRead ';
  ...
  // enum TDataSetState
  var TDataSetState =
  'dssEdit dssInsert dssBrowse dssInActive ';

  // enum TDateFormatType
  var TDateFormatType = 
  'dftDate dftShortDate dftDateTime dftTimeStamp ';

  // enum TDateOffsetType
  var TDateOffsetType = 
  'dotDays dotHours dotMinutes dotSeconds ';
  ...
  var TDeaAccessRights = 
  'arNone arView arEdit arFull ';

  // enum TDocumentDefaultAction
  var TDocumentDefaultAction =
  'ddaView ddaEdit ';
  ...
  var TEditorCloseObservType =
  'ecotFile ecotProcess ';
  ...
  var TEDocumentStorageFunction =
  'edsfExecutive edsfArchive ';

  // enum TEDocumentStorageType
  var TEDocumentStorageType =
  'edstSQLServer edstFile ';
  ...
  var TEDocumentVersionState =
  'vsDefault vsDesign vsActive vsObsolete ';
  ...
  var TExceptionCategory =
  'ecException ecWarning ecInformation ';

  // enum TExportedSignaturesType
  var TExportedSignaturesType =
  'estAll estApprovingOnly ';

  // enum TExportedVersionType
  var TExportedVersionType =
  'evtLast evtLastActive evtQuery ';
  ...
  // Перечисления
  var ENUMS = 
  TAccountType + TActionEnabledMode + TAddPosition + TAlignment + TAreaShowMode + 
  TCertificateInvalidationReason + TCertificateType + TCheckListBoxItemState + 
  TCloseOnEsc + TCompType + TConditionFormat + TConnectionIntent + TContentKind + 
  TControlType + TCriterionContentType + TCultureType + TDataSetEventType + 
  TDataSetState + TDateFormatType + TDateOffsetType + TDateTimeKind + TDeaAccessRights + 
  TDocumentDefaultAction + TEditMode + TEditorCloseObservType + TEdmsApplicationAction + 
  TEDocumentLockType + TEDocumentStepShowMode + TEDocumentStepVersionType + 
  TEDocumentStorageFunction + TEDocumentStorageType + TEDocumentVersionSourceType + 
  TEDocumentVersionState + TEncodeType + TExceptionCategory + TExportedSignaturesType + 
  TExportedVersionType + TFieldDataType + TFolderType + TGridRowHeight + THyperlinkType + 
  TImageFileFormat + TImageMode + TImageType + TInplaceHintKind + TISBLContext + 
  TItemShow + TJobKind + TJoinType + TLabelPos + TLicensingType + TLifeCycleStageFontColor + 
  TLifeCycleStageFontStyle + TLockableDevelopmentComponentType + TMaxRecordCountRestrictionType + 
  TRangeValueType + TRelativeDate + TReportDestination + TReqDataType + TRequisiteEventType + 
  TSBTimeType + TSearchShowMode + TSelectMode + TSignatureType + TSignerContentType + 
  TStringsSortType + TStringValueType + TStructuredObjectAttributeType + TTaskAbortReason + 
  TTextValueType + TUserObjectStatus + TUserType + TValuesBuildType + TViewMode + 
  TViewSelectionMode + TWizardActionType + TWizardFormElementProperty + TWizardFormElementType + 
  TWizardParamType + TWizardStepResult + TWizardStepType + TWorkAccessType + TWorkflowBlockType + 
  TWorkflowDataType + TWorkImportance + TWorkRouteType + TWorkState + TWorkTextBuildingMode;
