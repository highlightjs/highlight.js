...
function(hljs) {
  return {
    ...
    contains: [
      {
        className: 'operator',
        begin: '\\b(begin|end|start|commit|rollback|savepoint|lock|alter|create|drop|rename|call|delete|do|handler|insert|load|replace|select|truncate|update|set|show|pragma|grant|merge|describe)\\b(?!:)', // negative look-ahead here is specifically to prevent stomping on SmallTalk
