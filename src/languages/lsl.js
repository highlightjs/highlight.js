...
function(hljs) {
    return {
        ...
        contains: [
            ...
            {
                className: 'keyword',
                beginKeywords: 'state default'
            },
            {
                className: 'event',
                beginKeywords: 'at_rot_target at_target attach ' +
            ...
            },
            {
                ...
                beginKeywords: 'llAbs llAcos llAddToLandBanList llAddToLandPassList llAdjustSoundVolume llAgentInExperience llAllowInventoryDrop llAngleBetween llApplyImpulse llApplyRotationalImpulse llAsin llAtan2 llAttachToAvatar llAttachToAvatarTemp llAvatarOnLinkSitTarget llAvatarOnSitTarget llAxes2Rot llAxisAngle2Rot ' +
                    ...
                    'llMD5String llManageEstateAccess llMapDestination llMessageLinked llMinEventDelay llModPow llModifyLand llMoveToTarget ' +
                    'llNavigateTo ' +
                    'llOffsetTexture llOpenRemoteDataChannel llOverMyLand llOwnerSay ' +
                    'llParcelMediaCommandList llParcelMediaQuery llParseString2List llParseStringKeepNulls llParticleSystem llPassCollisions llPassTouches llPatrolPoints llPlaySound llPlaySoundSlave llPow llPreloadSound llPursue llPushObject ' +
            ...
            },
            {
                ...
                beginKeywords: 'EOF ' +
                    ...
                    'URL_REQUEST_DENIED URL_REQUEST_GRANTED',
                relevance: 10
            },
            {
                className: 'constant.vector',
                beginKeywords: 'TOUCH_INVALID_TEXCOORD TOUCH_INVALID_VECTOR ' +
                    'ZERO_VECTOR',
                relevance: 10
            },
            {
                className: 'constant.broken',
                beginKeywords: 'LAND_LARGE_BRUSH LAND_MEDIUM_BRUSH LAND_SMALL_BRUSH',
                relevance: 10
            },
            {
                className: 'constant.deprecated',
                beginKeywords: 'ATTACH_LPEC ATTACH_RPEC ' +
                    'DATA_RATING ' +
                    ...
                    'llClearExperiencePermissions llCloud ' +
                    'llGetExperienceList ' +
            ...
            },
            {
                ...
                beginKeywords: 'llGodLikeRezObject ' +
            ...
            },
            {
                ...
                beginKeywords: 'CHARACTER_MAX_ANGULAR_ACCEL CHARACTER_MAX_ANGULAR_SPEED CHARACTER_TURN_SPEED_MULTIPLIER ' +
                    ...
                    'llCollisionSprite ' +
                    'llPointAt ' +
                    'llRefreshPrimURL llReleaseCamera llRemoteLoadScript ' +
                    'llSetPrimURL llStopPointAt ' +
                    'llTakeCamera',
                relevance: 10
            },
            {
            ...
            { className: 'operator.increment', begin: '\\+\\+' },
            { className: 'operator.decrement', begin: '--' },
            { className: 'operator.shift.left', begin: '<<' },
            { className: 'operator.shift.right', begin: '>>' },
            { className: 'operator.comparison.less-equal', begin: '<=' },
            { className: 'operator.comparison.greater-equal', begin: '>=' },
            { className: 'operator.logical.and', begin: '&&' },
            { className: 'operator.logical.or', begin: '\\|\\|' },
            { className: 'operator.bit-wise.not', begin: '~' },
            { className: 'operator.bit-wise.xor', begin: '\\^' },
            { className: 'operator.bit-wise.and', begin: '&' },
            { className: 'operator.bit-wise.or', begin: '\\|' },
            { className: 'operator.comparison.equal', begin: '==' },
            { className: 'operator.comparison.not', begin: '!=' },
            { className: 'operator.logical.not', begin: '!' },
            { className: 'operator.comparison.less', begin: '<' },
            { className: 'operator.comparison.greater', begin: '>' },
            { className: 'operator.assignment.augmented', begin: '[%*+\\/\\-]=|=' },
            { className: 'operator.arithmetic', begin: '[%*+\\/\\-]' }
        ]
    };
}
