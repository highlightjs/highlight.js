...
export default function(hljs) {
  // Common pattern for identifier detection
  const UNDERSCORE_IDENT_RE = '[A-Za-zА-Яа-яёЁ_][A-Za-zА-Яа-яёЁ_0-9]+';

  // v7 unique keywords missing in v8
  const v7_keywords =
  'далее ';
  ...
  // v7 unique directives missing in v8
  const v7_meta_keywords =
  'загрузитьизфайла ';

  // v8 preprocessor instructions, compiler directives, annotations
  ...
  // meta-keyword : preprocessor instructions, compiler directives, annotations
  ...
  // v7 unique global context methods missing in v8
  ...
  // v8 system sets of values
  const v8_system_sets_of_values =
  'webцвета windowsцвета windowsшрифты библиотекакартинок рамкистиля символы цветастиля шрифтыстиля ';

  // v8 system enumerations - interface
  ...
  // v8 system enumerations - xml, json, xs, dom, xdto, web-services
  ...
  // v8 system enumerations - data composition system
  ...
  // built-in sets of values, system enumerations (contain child values, accessed via dereferencing)
  const CLASS =
  ...
  // v8 shared objects (objects have constructors, instances created via Новый)
  ...
  // v8 universal value collections
  ...
  /*
  wildcard: Asterisk for selecting all fields
  In queries - selects all fields
  ВЫБРАТЬ
    *
  ИЗ
  ...
  */
  ...
  // meta : preprocessor instructions, compiler directives
  const META = {
    className: 'meta',
  ...
  // function : procedure and function declarations
  const FUNCTION = {
    className: 'function',
    variants: [
      {
      ...
      },
      {
        ...
        begin: hljs.regex.concat(
          /(?:ЕСТЬNULL|ВЫРАЗИТЬ|РАЗНОСТЬДАТ|ДАТАВРЕМЯ|ГОД|МЕСЯЦ|ДЕНЬ|ДЕНЬГОДА|ДЕНЬНЕДЕЛИ|НЕДЕЛЯГОДА|ЧАС|МИНУТА|СЕКУНДА|НАЧАЛОПЕРИОДА|КОНЕЦПЕРИОДА|ДОБАВИТЬКДАТЕ|СУММА|КОЛИЧЕСТВО|МИНИМУМ|МАКСИМУМ|СРЕДНЕЕ)/,
