...
* ``ignore``: an array of class names used to prevent a code block from syntax highlighting when set to ``class`` attribute for ``<code>``, used for compatibility issues with Markdown which may fail to compile HTML with the default value ``no-highlight``.
