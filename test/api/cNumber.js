...
let pattern        = new RegExp(`${hljs.C_NUMBER_RE}$`);
let numberToString = utility.numberToString;

describe('.C_NUMBER_RE', function() {
  it('should match regular numbers', function() {
    let numbers = _.map(_.range(0, 1001), numberToString);

    numbers.should.matchEach(pattern);
  });

  it('should match decimals', function() {
    let decimal       = _.map(_.range(0, 1.001, 0.001), numberToString);
    let noLeadingZero = ['.1234', '.5206', '.0002', '.9998'];

    let numbers = [].concat(decimal, noLeadingZero);

    numbers.should.matchEach(pattern);
  });

  it('should match hex numbers', function() {
    let numbers = [ '0xbada55', '0xfa1755', '0x45362e', '0xfedcba'
